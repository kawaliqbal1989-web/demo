...
} from "../services/center-analytics.service.js";

function extractScope(req) {
  const tenantId = req.auth?.tenantId;
  const centerId = req.auth?.hierarchyNodeId;
  if (!tenantId || !centerId) return null;
  return { tenantId, centerId };
}
...
const analyticsAttendance = asyncHandler(async (req, res) => {
  ...
  if (!scope) return res.apiError(400, "Center scope missing", "CENTER_SCOPE_REQUIRED");
  const { limit, offset } = parsePagination(req.query);
  const { batchId, teacherUserId, studentId, from, to } = req.query;

  const result = await getAttendanceAnalytics({
    ...scope, batchId, teacherUserId, studentId, from, to, limit, offset
  });
...
const exportAttendanceCsv = asyncHandler(async (req, res) => {
  ...
  const result = await getAttendanceAnalytics({
    ...scope, batchId, teacherUserId, studentId, from, to, limit: 10000, offset: 0
  });
...
const analyticsWorksheets = asyncHandler(async (req, res) => {
  ...
  if (!scope) return res.apiError(400, "Center scope missing", "CENTER_SCOPE_REQUIRED");
  const { limit, offset } = parsePagination(req.query);
  const { batchId, teacherUserId, studentId, levelId, from, to } = req.query;

  const result = await getWorksheetAnalytics({
    ...scope, batchId, teacherUserId, studentId, levelId, from, to, limit, offset
  });
...
const exportWorksheetsCsv = asyncHandler(async (req, res) => {
  ...
  const result = await getWorksheetAnalytics({
    ...scope, batchId, teacherUserId, studentId, levelId, from, to, limit: 10000, offset: 0
  });
...
const analyticsStudentProgress = asyncHandler(async (req, res) => {
  ...
  if (!scope) return res.apiError(400, "Center scope missing", "CENTER_SCOPE_REQUIRED");
  const { limit, offset } = parsePagination(req.query);
  const { batchId, teacherUserId, studentId, levelId } = req.query;

  const result = await getStudentProgressAnalytics({
    ...scope, batchId, teacherUserId, studentId, levelId, limit, offset
  });
...
const exportStudentProgressCsv = asyncHandler(async (req, res) => {
  ...
  const result = await getStudentProgressAnalytics({
    ...scope, batchId, teacherUserId, studentId, levelId, limit: 10000, offset: 0
  });
