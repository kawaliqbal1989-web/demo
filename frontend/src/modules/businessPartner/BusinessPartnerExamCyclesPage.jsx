...
function BusinessPartnerExamCyclesPage() {
  ...
  const dashboardCards = useMemo(() => {
    ...
    return [
      ...
      { label: "Center Review", value: workflowQueue?.centerReview ?? 0, hint: "Teacher/center stage", tone: "#0284c7" },
      { label: "Franchise Review", value: workflowQueue?.centerSubmittedToFranchise ?? 0, hint: "Center submissions", tone: "#7c3aed" },
      { label: "BP Review", value: workflowQueue?.franchiseSubmittedToBusinessPartner ?? 0, hint: "Needs BP action", tone: "#9333ea" },
      { label: "Forwarded to Superadmin", value: workflowQueue?.businessPartnerSubmittedToSuperadmin ?? 0, hint: "Read-only at BP", tone: "#ea580c" },
      { label: "Approved Lists", value: workflowQueue?.approved ?? 0, hint: "Finalized lists", tone: "#16a34a" },
      { label: "Published Results", value: summary?.publishedResultCycles ?? 0, hint: "Visible to BP", tone: "#059669" }
    ];
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12, flexWrap: "wrap" }}>
        <div>
          ...
          <div style={{ fontSize: 13, color: "var(--color-text-muted)", marginTop: 4 }}>
            Review exam-cycle enrollment activity across your franchises and centers.
          </div>
        </div>
        <button
        ...
        <div className="card" style={{ display: "grid", gap: 12 }}>
          <div>
            ...
            <div style={{ color: "var(--color-text-muted)", fontSize: 13, marginTop: 4 }}>
              Counts are scoped to your business partner hierarchy.
            </div>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(170px, 1fr))", gap: 10 }}>
            {[
              ...
              ["Superadmin", "BP forwarded", workflowQueue.businessPartnerSubmittedToSuperadmin, "#ea580c"],
        ...
        columns={[
            ...
            render: (r) => {
              ...
              return (
                <div style={{ display: "grid", gap: 4 }}>
                  ...
                  <span style={{ color: "var(--color-text-muted)", fontSize: 12 }}>
                    {Number(summaryByCycle.franchiseSubmittedToBusinessPartner || 0) > 0 ? "BP action required" : "Read-only"}
                  </span>
                </div>
              );
            }
          ...
          },
          {
            ...
            render: (r) => {
              ...
              const hasPendingWork = Number(hierarchy.teacherSubmittedToCenter || 0)
                + Number(hierarchy.centerSubmittedToFranchise || 0)
                + Number(hierarchy.franchiseSubmittedToBusinessPartner || 0)
                + Number(hierarchy.businessPartnerSubmittedToSuperadmin || 0)
                + Number(hierarchy.centerReview || 0)
                + Number(hierarchy.approved || 0)
                > 0;

              return (
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                  {hasPendingWork ? (
                    <Link className="button secondary" style={{ width: "auto" }} to={`/bp/exam-cycles/${r.id}/pending`}>
                      Pending
                    </Link>
                  ) : null}
                  {String(r?.resultStatus || "").toUpperCase() === "PUBLISHED" ? (
                    <Link className="button secondary" style={{ width: "auto" }} to={`/bp/exam-cycles/${r.id}/results`}>
                      Results
                    </Link>
                  ) : (
                  ...
                  <Link className="button secondary" style={{ width: "auto" }} to={`/bp/exam-cycles/${r.id}/late-enrollment`}>
                    Late
                  </Link>
                </div>
              );
...
export { BusinessPartnerExamCyclesPage };
