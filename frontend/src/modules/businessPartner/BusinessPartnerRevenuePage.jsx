...
import { getMonthlyRevenue, getRevenueByCenter, getRevenueByType, getRevenueSummary } from "../../services/reportsService";
import { getFriendlyErrorMessage } from "../../utils/apiErrors";

function BusinessPartnerRevenuePage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const [s, t, c, m] = await Promise.all([
        ...
        getRevenueByCenter(params),
        getMonthlyRevenue(params)
      ]);

      setSummary(s.data);
      setByType(t.data.items || []);
      setByCenter(c.data.items || []);
  ...
  return (
    <section style={{ display: "grid", gap: 12 }}>
      ...
      <div className="card">
        <h3 style={{ marginTop: 0 }}>By Center</h3>
        <ul style={{ margin: 0, paddingLeft: 18 }}>
          {byCenter.map((row) => (
            <li key={row.centerId}>
              {row.name || row.centerId}: {row.grossAmount}
            </li>
          ))}
          {!byCenter.length ? <li style={{ color: "var(--color-text-muted)" }}>No data</li> : null}
        </ul>
      </div>
...
export { BusinessPartnerRevenuePage };
