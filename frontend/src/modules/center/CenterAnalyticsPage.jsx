...
function CenterAnalyticsPage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const [bRes, tRes, lRes, ecRes, cRes] = await Promise.allSettled([
        ...
        if (tRes.status === "fulfilled") setTeachers(extractItems(tRes.value));
  ...
  const buildParams = useCallback(() => {
    ...
    if (tab === "attendance" || tab === "worksheets" || tab === "mock-tests") {
      if (batchId) p.batchId = batchId;
    }
    if (tab === "attendance" || tab === "worksheets" || tab === "student-progress") {
      if (teacherUserId) p.teacherUserId = teacherUserId;
    }
    if (tab === "worksheets" || tab === "exams" || tab === "student-progress" || tab === "competitions") {
    ...
    return p;
  }, [tab, batchId, teacherUserId, levelId, examCycleId, competitionId, from, to, limit, offset]);
  ...
  const showTeacher = ["attendance", "worksheets", "student-progress"].includes(tab);
  ...
  return (
    <div style={{ maxWidth: 1200, margin: "0 auto" }}>
      <h2 style={{ marginBottom: 8 }}>📊 Center Analytics</h2>
      <div style={{ fontSize: 13, color: "var(--color-text-muted)", marginBottom: 16 }}>
        Multi-dimensional analytics across attendance, worksheets, tests, exams, competitions and student progress.
      </div>
      ...
      <div className="card" style={{ marginBottom: 16 }}>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 10, alignItems: "flex-end" }}>
          ...
          {showTeacher && (
            <label style={{ fontSize: 12 }}>
              Teacher
              <select value={teacherUserId} onChange={(e) => setTeacherUserId(e.target.value)} style={{ display: "block", marginTop: 4, minWidth: 140 }}>
                <option value="">All Teachers</option>
                {teachers.map((t) => (
                  <option key={t.id || t.authUserId} value={t.authUserId || t.id}>
                    {t.fullName || t.username || t.email || t.id}
                  </option>
                ))}
              </select>
            </label>
