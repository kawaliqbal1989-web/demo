...
function isCenterEditable(cycle) {
  return getCycleOwner(cycle) === "CENTER";
}
...
function CenterExamCyclesPage() {
  ...
  const dashboardCards = useMemo(() => {
    ...
    const centerReviewCount = Math.max(Number(workflowQueue.centerReview || 0) - Number(workflowQueue.teacherSubmittedToCenter || 0), 0);
    const cards = [
      ...
      { label: "Center Review", value: centerReviewCount, hint: "Center-owned draft/review", tone: "#0284c7" },
      { label: "Submitted to Franchise", value: workflowQueue?.centerSubmittedToFranchise ?? 0, hint: "Forwarded upward", tone: "#7c3aed" },
      { label: "Approved Lists", value: workflowQueue?.approved ?? 0, hint: "Finalized lists", tone: "#16a34a" },
      { label: "Published Results", value: summary?.publishedResultCycles ?? 0, hint: "Visible to center", tone: "#059669" }
    ];

    if (Number(workflowQueue?.teacherSubmittedToCenter || 0) > 0) {
      cards.splice(2, 0, {
        label: "Teacher Review",
        value: workflowQueue.teacherSubmittedToCenter,
        hint: "Teacher submissions pending center",
        tone: "#0ea5e9"
      });
    }

    if (Number(workflowQueue?.franchiseSubmittedToBusinessPartner || 0) > 0 || Number(workflowQueue?.businessPartnerSubmittedToSuperadmin || 0) > 0) {
      cards.splice(cards.length - 2, 0, {
        label: "Forwarded Above",
        value: Number(workflowQueue?.franchiseSubmittedToBusinessPartner || 0) + Number(workflowQueue?.businessPartnerSubmittedToSuperadmin || 0),
        hint: "Beyond center ownership",
        tone: "#ea580c"
      });
    }
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12, flexWrap: "wrap" }}>
        <div>
          ...
          <div style={{ fontSize: 13, color: "var(--color-text-muted)", marginTop: 4 }}>
            Manage Center enrollment, review Teacher submissions, and forward eligible lists to your Franchise.
          </div>
        </div>
        <button
        ...
        <div className="card" style={{ display: "grid", gap: 12 }}>
          <div>
            ...
            <div style={{ color: "var(--color-text-muted)", fontSize: 13, marginTop: 4 }}>
              Counts are scoped to your center enrollment workflow.
            </div>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(170px, 1fr))", gap: 10 }}>
            {[
              ["Teacher", "Teacher submissions", workflowQueue.teacherSubmittedToCenter, "#0ea5e9"],
              ["Center", "Center-owned review", Math.max(Number(workflowQueue.centerReview || 0) - Number(workflowQueue.teacherSubmittedToCenter || 0), 0), "#0284c7"],
              ["Franchise", "Center submissions", workflowQueue.centerSubmittedToFranchise, "#7c3aed"],
              ["Business Partner", "Franchise submissions", workflowQueue.franchiseSubmittedToBusinessPartner, "#9333ea"],
              ["Superadmin", "BP submissions", workflowQueue.businessPartnerSubmittedToSuperadmin, "#ea580c"],
        ...
        <DataTable
          ...
          { key: "name", header: "Name" },
          {
            ...
            render: (r) => {
              ...
              const centerEditable = isCenterEditable(r);
              return (
                <div style={{ display: "grid", gap: 4 }}>
                  ...
                  <span style={{ color: "var(--color-text-muted)", fontSize: 12 }}>
                    {centerEditable ? "Center action required" : "Read-only after forwarding"}
                  </span>
                </div>
              );
            }
