...
const ACTION_LABELS = {
  ...
  ESCALATE_TO_FRANCHISE: "Escalate To Franchise",
...
const ACTION_DIALOGS = {
  REVIEW: {
    title: "Review Academic Workflow",
    description: "Capture review notes for immutable governance history.",
    fields: [{ name: "notes", label: "Notes", type: "textarea", required: false }]
  },
  ACKNOWLEDGE: {
    title: "Acknowledge Academic Issue",
    description: "Use acknowledgement when the issue is understood and under center governance.",
    fields: [{ name: "notes", label: "Notes", type: "textarea", required: false }]
  },
  START_RECOVERY: {
    title: "Start Recovery Workflow",
    description: "Track attendance recovery, grading recovery, or teacher coordination work with a due date.",
    fields: [
  ...
  SCHEDULE_FOLLOW_UP: {
    ...
    description: "Create an accountable follow-up checkpoint with immutable notes.",
    fields: [
      { name: "reason", label: "Follow-up Reason", type: "textarea", required: false },
      { name: "notes", label: "Notes", type: "textarea", required: false },
      { name: "taskDueAt", label: "Due At", type: "datetime-local", required: false }
    ]
  },
  ESCALATE_TO_FRANCHISE: {
    title: "Escalate To Franchise",
    description: "Escalation records immutable governance history and creates a franchise review task.",
    fields: [
  ...
  RESOLVE: {
    title: "Resolve Academic Issue",
    description: "Resolve closes the workflow while preserving the full governance timeline.",
    fields: [{ name: "notes", label: "Resolution Notes", type: "textarea", required: false }]
  },
  REOPEN: {
    title: "Reopen Academic Issue",
    description: "Reopen returns the workflow to the active governance queue with a new version.",
...
async function runWorkflowAction(actionType, workflowId, payload) {
  switch (actionType) {
    case "REVIEW":
      return reviewCenterWorkflow(workflowId, payload);
    case "ACKNOWLEDGE":
      return acknowledgeCenterWorkflow(workflowId, payload);
    case "START_RECOVERY":
      return startCenterWorkflowRecovery(workflowId, payload);
    case "SCHEDULE_FOLLOW_UP":
      return scheduleCenterWorkflowFollowUp(workflowId, payload);
    case "ESCALATE_TO_FRANCHISE":
      return escalateCenterWorkflow(workflowId, payload);
    case "RESOLVE":
      return resolveCenterWorkflow(workflowId, payload);
    case "REOPEN":
      return reopenCenterWorkflow(workflowId, payload);
    default:
...
function PreviewBlock({ metadata }) {
  ...
  const sections = [
    ...
    { key: "backlogPreview", label: "Worksheet Backlog" },
    { key: "delayedReviewPreview", label: "Delayed Reviews" },
    { key: "inactiveTeachers", label: "Inactive Teachers" },
    { key: "batchRisks", label: "Batch Risks" }
  ];
        ...
        return (
          <div key={section.key} style={{ display: "grid", gap: 6 }}>
            ...
            {items.map((item, index) => (
              <div key={`${section.key}-${item.id || item.studentId || item.batchId || item.teacherUserId || index}`} style={{ fontSize: 13, color: "var(--color-text-muted)" }}>
                {item.studentName || item.batchName || item.teacherName || item.title || item.name || item.id}
                {item.attendanceRate !== undefined ? ` · Attendance ${item.attendanceRate}%` : ""}
                {item.worksheetCompletionRate !== undefined ? ` · Worksheet ${item.worksheetCompletionRate}%` : ""}
                {item.inactiveDays !== undefined && item.inactiveDays !== null ? ` · Inactive ${item.inactiveDays}d` : ""}
                {item.operationalHealthScore !== undefined ? ` · Health ${item.operationalHealthScore}` : ""}
              </div>
            ))}
          </div>
        );
...
function CenterWorkflowDetailPage() {
  const { id } = useParams();
  ...
  useEffect(() => {
    ...
    Promise.all([getCenterWorkflowDetail(id), getCenterWorkflowHistory(id, { limit: 50, offset: 0 })])
  ...
  if (loading && !detail) {
    return <LoadingState label="Loading center workflow detail..." />;
  }
    ...
    <section style={{ display: "grid", gap: 16 }}>
      <div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "flex-start", flexWrap: "wrap" }}>
        <div style={{ display: "grid", gap: 8 }}>
          ...
          <div style={{ fontSize: 13, color: "var(--color-text-muted)" }}>
            Workflow type: {workflow.workflowType} · Last detected {formatDateTime(workflow.lastDetectedAt)}
          </div>
        </div>
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
          <button className="button secondary" type="button" style={{ width: "auto" }} onClick={() => setRefreshTick((tick) => tick + 1)}>
            Refresh
          </button>
          <Link className="button secondary" style={{ width: "auto" }} to="/center/workflows">
            Back To Queue
          </Link>
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 12 }}>
        <div className="card" style={{ display: "grid", gap: 10 }}>
          ...
          <div style={{ fontWeight: 700 }}>{center?.name || workflow.centerName || "Center scope"}</div>
          <div style={{ color: "var(--color-text-muted)" }}>{center?.code || workflow.centerCode || workflow.centerId}</div>
        ...
        <div className="card" style={{ display: "grid", gap: 10 }}>
          ...
          <div style={{ display: "grid", gap: 6, color: "var(--color-text-muted)" }}>
            <div>Metric: {workflow.metadata?.anomaly?.metricKey || workflow.metadata?.anomaly?.metric || workflow.metricKey || "-"}</div>
            <div>Observed: {workflow.metadata?.anomaly?.observedValue ?? "-"}</div>
            <div>Threshold: {workflow.metadata?.anomaly?.threshold ?? "-"}</div>
            <div>Acknowledged: {formatDateTime(workflow.acknowledgedAt)}</div>
            <div>In progress: {formatDateTime(workflow.inProgressAt)}</div>
            <div>Follow-up: {formatDateTime(workflow.followUpAt)}</div>
          </div>
        </div>
        ...
        <div className="card" style={{ display: "grid", gap: 10 }}>
          ...
          {!workflow.allowedActions?.length ? (
            <div style={{ color: "var(--color-text-muted)" }}>No center actions are available in the current workflow state.</div>
          ) : (
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(320px, 1fr))", gap: 12 }}>
        <div className="card" style={{ display: "grid", gap: 12 }}>
          <div style={{ fontWeight: 700, fontSize: 18 }}>Operational Preview</div>
        ...
        <div className="card" style={{ display: "grid", gap: 12 }}>
          ...
          {tasks.length ? (
            ...
            <div style={{ color: "var(--color-text-muted)" }}>No active or historical tasks were returned for this workflow.</div>
          )}
        </div>
      </div>
...
export { CenterWorkflowDetailPage };