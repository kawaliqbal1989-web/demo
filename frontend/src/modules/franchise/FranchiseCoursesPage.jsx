...
import { listFranchiseCourses } from "../../services/franchiseService";

function FranchiseCoursesPage() {
  ...
  const load = async () => {
    ...
    setError("");
    try {
      const resp = await listFranchiseCourses();
      setRows(resp?.data || []);
    } catch (err) {
  ...
  return (
    <section style={{ display: "grid", gap: 12 }}>
      ...
      <div className="card">
        ...
        {!rows.length ? <div style={{ color: "var(--color-text-muted)", marginTop: 10 }}>No courses available for your franchise.</div> : null}
      </div>
    </section>
  );
}

export { FranchiseCoursesPage };
