...
const saGetCenterDetail = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const effectiveBranding = await resolveBrandingForCenter(center.id, req.auth.tenantId).catch(() => null);

    return res.apiSuccess("Center detail loaded", {
      ...center,
      address: null,
      brandingApprovedBy: null,
      authUser,
      franchiseProfile,
      effectiveBranding,
      metrics: { studentsCount, teachersCount, batchesCount },
    ...
    metadata: {
      before: {
        ...
        try {
          ...
          return res.apiSuccess("Center branding updated", {
            ...updated,
            effectiveBranding: await resolveBrandingForCenter(updated.id, req.auth.tenantId).catch(() => null)
          });
        } catch (error) {
          const fallbackCenter = await prisma.centerProfile.findFirst({
            where: {
              tenantId: req.auth.tenantId,
              OR: [{ id }, { authUserId: id }]
            },
          ...
          return res.apiSuccess("Center branding updated", {
            ...
            skipped: true,
            reason: "CENTER_BRANDING_FALLBACK"
          });
        }
