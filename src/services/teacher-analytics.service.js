...
async function getAttendanceAnalytics({ tenantId, centerId, teacherUserId, batchId, studentId, from, to, limit, offset }) {
  ...
  const items = (Array.isArray(rows) ? rows : []).map((row) => {
    ...
    return {
      ...
      batchName: row.batchNames ? String(row.batchNames) : null,
      totalSessions: ts,
      presentCount: pc,
...
async function getWorksheetAnalytics({ tenantId, centerId, teacherUserId, batchId, studentId, levelId, from, to, limit, offset }) {
  const conditions = [
    Prisma.sql`s.tenantId = ${tenantId}`,
    Prisma.sql`s.hierarchyNodeId = ${centerId}`,
...
async function getMockTestAnalytics({ tenantId, centerId, teacherUserId, batchId, from, to, limit, offset }) {
  const conditions = [
...
async function getExamAnalytics({ tenantId, centerId, teacherUserId, examCycleId, levelId, from, to, limit, offset }) {
  const conditions = [
    Prisma.sql`s.tenantId = ${tenantId}`,
    Prisma.sql`s.hierarchyNodeId = ${centerId}`,
    Prisma.sql`s.currentTeacherUserId = ${teacherUserId}`
...
async function getCompetitionAnalytics({ tenantId, centerId, teacherUserId, competitionId, levelId, from, to, limit, offset }) {
  const conditions = [
    Prisma.sql`s.tenantId = ${tenantId}`,
    Prisma.sql`s.hierarchyNodeId = ${centerId}`,
    Prisma.sql`s.currentTeacherUserId = ${teacherUserId}`
...
async function getStudentProgressAnalytics({ tenantId, centerId, teacherUserId, batchId, studentId, levelId, limit, offset }) {
  const conditions = [
    Prisma.sql`s.tenantId = ${tenantId}`,
    Prisma.sql`s.hierarchyNodeId = ${centerId}`,
    Prisma.sql`s.isActive = 1`,
