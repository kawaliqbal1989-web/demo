...
const run = async () => {
  const login = await req('/auth/login', {
    ...
    body: JSON.stringify({ tenantCode: 'DEFAULT', username, password: 'Pass@123' })
  });

  if (!login.ok) {
    console.log(JSON.stringify({ step: 'login_failed', username, login }, null, 2));
    process.exit(1);
  }
  ...
  if (!studentId) {
    console.log(JSON.stringify({ step: 'no_student_found', username, students }, null, 2));
    process.exit(2);
  }
